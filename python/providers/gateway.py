...
class Gateway:
    ...
    def _dedup_lock(self, key: str) -> threading.Lock:
        with self._guard:
    ...
    def request(
        ...
        *,
        ...
        """Return (body_text, from_cache). `send` performs the actual HTTP call."""
        with self._dedup_lock(key):
            ...
            body = response.text
            self.cache.put(key, policy.name, body, ttl_seconds)
            return body, False
